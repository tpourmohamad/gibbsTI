...
double bootstrap_coverage_content_cpp(double tl,
  ...
  // FIX: Look inside gibbsTI namespace (not global env)
  ...
  for(int b = 0; b < B; b++) {
    ...
    NumericMatrix post = sample_joint_cpp(boots(_, b), tl, tu, eta);
...
List calibrate_eta_joint_content_cpp(NumericVector y,
  ...
  for(int s = 1; s <= max_iter; s++) {
    ...
    double cover = bootstrap_coverage_content_cpp(tau_lower, tau_upper, alpha, B, eta, boots, y);
    ...
    double step = 0.5 / pow(1.0 + s, 0.75);
