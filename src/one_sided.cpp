...
//' @export
 ...
 double bootstrap_coverage_cpp(double tau, double alpha, int B, double eta, double theta_hat, NumericMatrix boots) {
   ...
   for(int b = 0; b < B; b++) {
     ...
     // Use smaller n for bootstrap speed
     NumericVector boot_posterior = slice_sample_1d_cpp(Rcpp::median(boot_data), boot_data, tau, eta, 0.5, 1000, n);
...
//' @export
 ...
 List calibrate_eta_cpp(NumericVector y, double tau, double alpha = 0.1, double eta0 = 1.0,
                        ...
                        double c = 0.5, double gamma = 0.75, bool verbose = true) {
   int n = y.size();

   // Initial estimate to serve as "truth" for bootstrap
   NumericVector initial_posterior = slice_sample_1d_cpp(Rcpp::median(y), y, tau, 1.0, 0.5, 1000, 4000);
   ...
   for(s = 1; s <= max_iter; s++) {
     ...
     double est_coverage = bootstrap_coverage_cpp(tau, alpha, B, eta_current, theta_hat, boots);
